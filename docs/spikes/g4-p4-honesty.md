...
Documenting G4-P4 as **Not run** does **not** close live G2–G4 or RB-011, and does **not** prove containment on Thor. Chronomancer opened live work on **Thor SSH networked LLM** with paid cloud hard cap **$0**; Mac smoke Pass is **wiring only**. SSH ≠ G4 containment. Secrets stay in gitignored `.env`. This note stays offline honesty only — no invented jail Pass, no secrets in docs, no BOARD ownership fight.

## Archivist verification

**Verified** (Mnemosyne Archivist, 2026-09-22 ~3:28 PM ET, tip `8b1679b` / post-#42 + #44):

| Check | Result |
| --- | --- |
| In-repo worker jail / mount module | **Absent** |
| Negative FS-denial artifact | **Absent** |
| G4-P7 OS inventory | Present — documentation only |
| `scripts/spikes/g4-p4-grade.mjs` | Refuses **Pass** without jail + negative transcript |
