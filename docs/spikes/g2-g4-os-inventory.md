...
| Operator HTTP reachable without token? | **Yes on current control API** | `apps/server` CORS-lists `x-rulebreak-operator-token` but does not enforce it on `POST /api/campaigns` (G4-P5 Fail) |
...
1. Enforce operator token on control API (closes G4-P5 Fail).
